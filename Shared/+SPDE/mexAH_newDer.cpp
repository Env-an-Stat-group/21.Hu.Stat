...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	if(nrhs != 12)
	{
		mexErrMsgTxt("Twelve (12) input arguments required.");
	}
	...
	makeAH(iVec, jVec, vVec, cLoc, vLoc, tt, tv, vt, vx, vy, vz, needDiff, vxD, vyD, vzD);
...
void makeAH(std::vector<double>& iVec, std::vector<double>& jVec, std::vector<double>& vVec, const std::vector<Point3d>& cLoc, const std::vector<Point3d>& loc, const std::vector<Point3>& tt, const std::vector<Point3>& tv, const std::vector<std::vector<int> >& vt, double* vxa, double* vya, double* vza, double needDiff, double* vxDa, double* vyDa, double* vzDa){
	...
	for(unsigned int tr = 0; tr < tt.size(); ++tr){
		...
		for(unsigned int edg = 0; edg < 3; ++edg){
			...
			double A = triArea(xS, xE, xN)+triArea(xN, xW, xS);
			...
			Point2d vecDF = Point2d(e1.dot(vecDField), e2.dot(vecDField));
			...
			Point2d n = Point2d(yN.y, 0.0);

			// Calculate help variables
			double wSE = (n.x*(H11*(yE.y-yS.y)-H12*(yE.x-yS.x))+n.y*(H12*(yE.y-yS.y)-H22*(yE.x-yS.x)))/(2.0*A);
			double wEN = (n.x*(H11*(yN.y-yE.y)-H12*(yN.x-yE.x))+n.y*(H12*(yN.y-yE.y)-H22*(yN.x-yE.x)))/(2.0*A);
			double wNW = (n.x*(H11*(yW.y-yN.y)-H12*(yW.x-yN.x))+n.y*(H12*(yW.y-yN.y)-H22*(yW.x-yN.x)))/(2.0*A);
			double wWS = (n.x*(H11*(yS.y-yW.y)-H12*(yS.x-yW.x))+n.y*(H12*(yS.y-yW.y)-H22*(yS.x-yW.x)))/(2.0*A);
